...
| `StatusBadge.vue` | The status pill: tinted fill, hairline, dot, label. One lookup into the T1 triplets. | props `status: StatusKey`, `label?: string`, `size?: 'sm' \| 'md' = 'md'` · exports `type StatusKey`, `labelFor(status)` | Every status, everywhere. There is deliberately **no colour prop** — the status *is* the colour. |
