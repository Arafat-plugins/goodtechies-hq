...
| `Tasks/TaskBoard.vue` | Eight lanes in lifecycle order with counts, drag between and within, and the wrapped jump rail. | props `board: BoardPayload`, `transitions`, `filters`, `surface`, `statuses`, `priorities`, `projects`, `tags`, `employees?`, `searchPlaceholder`, `emptyTitle`, `emptyDescription` | The Board pages. A drop between lanes is `POST …/status` with `after_id`; inside one it is `POST …/reorder`. It mounts **one** headless `TaskStatusActions` re-bound per move rather than one per card — the dialogs are modal, and forty mounted focus traps is thirty-nine too many — and gives focus back by **card id**, because the optimistic move has already re-rendered the card that opened the dialog. Lanes are `w-72` and the strip is the only thing that scrolls sideways. **From `md` up the board is a fixed region and the page does not scroll at all:** the strip is given a *measured* height — what the viewport has left under the page header, less the `<main>` gutter and the ordering note below it, recomputed on resize and whenever `<main>` changes size — each lane fills it with its heading outside the scroller and its cards scrolling inside, and the strip's horizontal scrollbar therefore sits inside the viewport rather than under every card. Below `md`, and on any window that would leave the strip under 10 rem, nothing is measured and the page scrolls as before: that is the 360 answer (one snapped column at a time under the wrapped status rail) and it is not to be undone. The strip's **background pans by drag** — pointer events, `grab`/`grabbing`, and never started on a card, a link, a button or a menu, so a card's HTML5 drag and a pan cannot fight; a grab that never moved is still a click, and touch is left to the browser's own scrolling. |
