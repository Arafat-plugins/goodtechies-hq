...
| C-4 | The Board strip is a **measured region that fills the viewport**: lanes scroll vertically inside themselves, the page does not scroll, and the horizontal scrollbar is always on screen | The client reported the scrollbar as unreachable, and it was — lanes grew to their contents, the page ran to 1858px, and the scrollbar sat 900px below the fold. Now 0px of page scroll at 1280. Below `md` nothing changed: the one-column snap and the status rail still answer the narrow case |
| C-5 | Grabbing empty board background pans it, with cards, buttons, links and menus excluded from starting a pan | The client asked for it by name. The exclusion is what stops a pan and a card drag fighting; a click that did not move is still delivered as a click |
