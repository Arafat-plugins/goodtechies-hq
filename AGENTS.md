...
Measured 2026-09-27 at Phase 6's core (conversations, messages, realtime, Team) with
`php vendor/bin/pest`: none failing (**1585 passed, ~8430 assertions**).
`vendor/bin/pint --test`: passed. `npx vue-tsc --noEmit`: passed. `npm run build`: passed. If
your number is not 1585, that is a finding, not drift.
